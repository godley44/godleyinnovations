...
- **Token rotation**: the Supabase access token behind `SUPABASE_ACCESS_TOKEN` was created **Sep 17, 2026** and expires after 90 days (**~Dec 16, 2026**). Rotate it before then: create a new token at supabase.com → Account → Access Tokens, update the `SUPABASE_ACCESS_TOKEN` repo secret (and the Claude environment credential), then re-run deploy-on-main to verify. A reminder is scheduled for Dec 10, 2026.
